...
def embeddings_quality(embeddings_file, configuration):
    ...
    # wv = models.KeyedVectors.load(embeddings_file)
    sum_total = 0
    count_tests = 0
    result_col = {}
    result_con = {} 
    ...
    if len(nmcon_tests) > 0:
        ...
        sum_total += result_con['MS_avg']
        count_tests += 1
        print('# MS_avg: {:.2f}'.format(result_con['MS_avg']))
    else:
    ...
    _r = ['MA_avg', 'MR_avg', 'MS_avg', 'EQ_avg']
